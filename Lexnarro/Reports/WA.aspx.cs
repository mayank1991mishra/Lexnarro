                ...
                using (SqlCommand cmd = new SqlCommand())
                {
                    using (SqlDataAdapter adap = new SqlDataAdapter())
                    {
                        using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["LaxNarroConnectionString"].ConnectionString))
                        {
                            ...
                            cmd.CommandText = @"SELECT  User_Training_Transaction.Date, User_Training_Transaction.Hours, 
User_Training_Transaction.Provider, User_Training_Transaction.Financial_Year, User_Training_Transaction.Forwardable, 
                         User_Training_Transaction.Has_been_Forwarded, User_Training_Transaction.Descrption, 
User_Training_Status.Received_By_Forwarding, User_Training_Status.Units_Done, 
                         User_Training_Status.Min_Required_Category_Units, User_Profile.FirstName, 
User_Profile.LastName, User_Profile.OtherName, User_Profile.StreetNumber, User_Profile.StreetName, User_Profile.PostCode, 
                         User_Profile.Suburb, User_Profile.LawSocietyNumber, User_Profile.EmailAddress,
User_Profile.PhoneNumber, User_Profile.Address, User_Training_Transaction.Your_Role, Country_Master.Name AS 'Country Name', 
...
Activity_Master.ShortName AS 'ActivityShortName', Category_Master.Name  AS 'Category Name', Category_Master.ShortName as 'CategShortName',
                         Sub_Activity_Master.Name AS 'SubActivity Name', Sub_Activity_Master.ShortName AS 'SubActivityShortName', 
                         User_Training_Transaction.Your_Role
